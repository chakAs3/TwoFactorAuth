var app = angular.module('Multilingual', ['pascalprecht.translate','ngRoute','ngCookies','ngSanitize','btford.socket-io' ,'algoliasearch']);//i use ngSanitize to sanitize user input in case of rendering it
...
app.controller('MainController', ['$scope','$http','algolia', function($scope,$http,algolia) {
  // get all users
      $http.get('/api/users')
      ...
      // search on JadoPado using RESTfull API
      $scope.search = {
        query: '',
        hits: []
      };
      var client = algolia.Client('BHFNGI9C92', '170f22b70d7e9ad72b1fc2991812cb2e');
      var index = client.initIndex('products');
      var last_query = $scope.search.query

      $scope.$watchGroup(['search.query','search.page'], function(newValues, oldValues, scope) {
         //console.log("oldValues :"+$scope.search.query+",  newValues :"+newValues[0]);
         var page =  $scope.search.page ;
          if(last_query != newValues[0]) page = 0 ;
         last_query = $scope.search.query ;

        // reset page if it is new query

        index.search($scope.search.query,{hitsPerPage: 20,page:page})
          .then(function searchSuccess(content) {
            console.log(content);

            // add content of search results to scope for display in view
            $scope.search.page = content.page;
            $scope.search.nbPages = content.nbPages;
            $scope.search.hitsPerPage = content.hitsPerPage;
            $scope.search.nbHits = content.nbHits;
...
// searching product on JadoPado
app.controller('SearchCtrl', ['$scope', 'algolia', function($scope, algolia) {
      $scope.search = {
        query: '',
        hits: []
      };
      var client = algolia.Client('BHFNGI9C92', 'bb61e5ff583c0f07f7d30cf4a5402fad');
      var index = client.initIndex('products');
